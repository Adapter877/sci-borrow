        ...
        if ($login) {
            ...
            \Index\Home\Controller::renderCard($card, 'icon-warning', $login['name'], number_format($items->returned), ' {LNG_Un-Returned items}', 'index.php?module=borrow-setup&amp;status=2&amp;due=1');
                if (isset($items->allpending)) {
                    \Index\Home\Controller::renderCard($card, 'icon-warning', '-', number_format($items->confirmed), 'รายการยืม-ที่ได้รับการอนุมัติแล้ว', 'index.php?module=borrow-report&amp;status=2');
                    \Index\Home\Controller::renderCard($card, 'icon-exchange', '-', number_format($items->allpending), 'รายการยืม-รอตรวจสอบ', 'index.php?module=borrow-report&amp;status=0');
                    \Index\Home\Controller::renderCard($card, 'icon-exchange', '-', number_format($items->returned), 'รายการที่คืนแล้ว', 'index.php?module=borrow-report&amp;status=3');
                    \Index\Home\Controller::renderCard($card, 'icon-exchange', '-', number_format($items->returned), 'รายการครบกำหนดคืน', 'index.php?module=borrow-report&amp;status=2&due=1');
                }
        }
    }
}
